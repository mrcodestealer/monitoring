...
默认 ``GRAFANA_SCREENSHOT_FULL_PAGE=1`` 时仍优先对 ``main`` 元素截图（避免 full_page 拼接出现重复顶栏/大块黑缝）；失败才回退 viewport/full_page。
    ...
    try:
        ...
        h = page.evaluate(
            """() => {
              ...
              return Math.min(Math.max(h, 800), 24000);
            }"""
        )
        h = int(h or 800)
        y = 0
...
def _grafana_playwright_capture_dashboard_png(page: Any) -> bytes:
    ...
    for sel in (
        ...
        try:
            ...
            if not box or float(box.get("height") or 0) < 180:
                continue
            png = loc.screenshot(type="png", animations="disabled")
            if png and len(png) > 4000:
                logger.info(
                    "Grafana screenshot: element capture via %r height=%.0f bytes=%s",
                    sel,
            ...
            logger.debug("Grafana screenshot: element %r capture failed: %s", sel, e)
    full_page = _lark_env_truthy("GRAFANA_SCREENSHOT_FULL_PAGE")
    logger.info("Grafana screenshot: falling back to page screenshot full_page=%s", full_page)
    try:
