...
_CFG: Dict[str, Any] = {
    ...
    # 1=在 monitoring 交互卡片底部增加「刷新截图」按钮（点击后异步再发一张最新 PNG）
    "MONITORING_MESSAGE_CARD_REFRESH_BUTTON": "1",
...
def _monitoring_interactive_card_dict(
    ...
    if _lark_env_truthy("MONITORING_MESSAGE_CARD_REFRESH_BUTTON"):
        elements.append(
            {
                "tag": "action",
                "actions": [
                    {
                        ...
                        "value": {"action": "monitoring_refresh_screenshot"},
                    }
                ],
            }
        )
...
def _lark_verify_event_token(data: Dict[str, Any]) -> bool:
    ...
    if isinstance(et, str) and et.startswith("card.action"):
        ...
        if _lark_looks_like_lark_card_update_credential(raw_tok):
            # ``card.action`` webhook may use short-lived card credential token (c-/d-), not app verification token.
            return True
...
def _lark_card_action_ids(data: Dict[str, Any]) -> Tuple[str, str]:
    ev = data.get("event") if isinstance(data.get("event"), dict) else {}
...
def _monitoring_send_screenshot_only_worker(chat_id: str, open_id: str) -> None:
    try:
        ...
        if (chat_id or "").strip():
            ...
            raise RuntimeError("missing chat_id/open_id for card refresh screenshot")
        logger.info(
            "monitoring card refresh screenshot sent chat=%r open=%r bytes=%s",
            bool(chat_id),
            bool(open_id),
            len(png),
        )
    except Exception as e:
        logger.exception("monitoring card refresh screenshot failed")
        err = f"刷新截图失败：{e}"
        try:
            if (chat_id or "").strip():
                _lark_send_text("chat_id", chat_id.strip(), err)
            elif (open_id or "").strip():
                _lark_send_text("open_id", open_id.strip(), err)
        except Exception:
            logger.exception("monitoring card refresh error message send failed")


def _handle_lark_card_action_event(data: Dict[str, Any]) -> None:
    et = _lark_header_event_type(data)
    if not (isinstance(et, str) and et.startswith("card.action")):
        return
    val = _lark_card_action_value(data)
    action = _lark_dict_pick_str(val, "action", "cmd")
    if action != "monitoring_refresh_screenshot":
        logger.info("card.action ignored action=%r", action or None)
        return
    ...
    with _monitoring_reply_dispatch_lock:
        ...
        if ev_id:
            ...
            if len(_monitoring_card_action_event_ids) > 2000:
                ...
                _monitoring_card_action_event_ids.add(ev_id)
    chat_id, open_id = _lark_card_action_ids(data)
    logger.info(
        "card.action accepted action=%r chat=%r open=%r event_id=%r",
        action,
        bool(chat_id),
        bool(open_id),
        ev_id or None,
    ...
    threading.Thread(
        target=_monitoring_send_screenshot_only_worker,
        args=(chat_id, open_id),
        daemon=True,
        name="monitoring-card-refresh",
    ).start()
...
def webhook_event():
    ...
    if et_l.startswith("card.action"):
        try:
            _handle_lark_card_action_event(data)
        except Exception:
