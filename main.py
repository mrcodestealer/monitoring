...
默认 ``GRAFANA_SCREENSHOT_FULL_PAGE=1``：``/mo`` 无告警时整页截图。``GRAFANA_SCREENSHOT_ALERT_SOLO_PANEL=1``（默认）时 **告警** 对每个触发面板各走 ``/d-solo/…?panelId=`` 截一张（视口 PNG），多告警会发多张图；整页回退时仍一张 dashboard + 多红框。
...
def _grafana_headless_screenshot_png_list(
    ...
    *,
    ...
    alerting panel via ``/d-solo/{uid}/{slug}?panelId=…`` (viewport per panel).
                ...
                if not _lark_env_truthy("GRAFANA_SCREENSHOT_ENABLE"):
                    ...
                    if su <= 0 or eu <= 0:
                        ...
                        try:
                            ...
                            highlight = (
                                _monitoring_alert_panel_titles(payload) if alert_hit else []
                            )
                            pngs = _grafana_headless_screenshot_png_list(
                                grafana_session,
                                su,
