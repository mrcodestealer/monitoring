...
_health_report_import_error = ""
try:
    import health_report  # shared daily health card (health_report.py next to this file)
except Exception as _e:  # a missing / broken copy must never stop the bot from booting
    health_report = None  # type: ignore[assignment]
    _health_report_import_error = f"{_e.__class__.__name__}: {_e}"  # logged by the starter (no logger yet)
...
_CFG: Dict[str, Any] = {
    ...
    "MONITORING_ERROR_REQ_SERIES_MIN_SPIKE": (
        ...
        "fpms-nt-ali-prod-promotion-rollout|GetRemainingChances=150;"
        "fpms-nt-ali-prod-promotion-rollout|ClaimPlayerPalayokBlast=150;"
        "fpms-nt-ali-prod-promotion-rollout|CheckAndCreateMission=150;"
        "fpms-nt-ali-prod-promotion-rollout|GetRecentMilyonaryoWinners=150;"
        "fpms-nt-ali-prod-promotion-rollout|AcknowledgeJackpotWin=150;"
        "fpms-nt-ali-prod-promotion-rollout|grpc.promotion.v1.FrontendLuckyCoinsService/GetRebateBonusPopup=150;"
        "fpms-nt-ali-prod-promotion-rollout|grpc.promotion.v1.FrontendLuckyCoinsService/GetQuickTaskPopup=150;"
        "igo-sw-http-main-apisix-pp-hypergrid|POST /refund=150;"
        "igo-sw-general-prod-ali-igo-sw-cluster-route|POST /=150;"
        "igo-sw-jili-prod-ali-igo-sw-cluster-route|POST /=150;"
        "igo-prod-ali-igo-sw-internal|POST /fpms/request=150"
    ),
    ...
    # 模型常驻（见 WARM）后正常判定只需数秒；给 120s 余量。注意该超时会「阻塞告警发送」，
    # 设太大 = 告警延迟数分钟才进群，故不再用 300s。
    ...
    # 0=关闭全部自动发送（手动 /freespin、/coremetrics 仍可用）
    ...
    # 这些群**永不**接收「菜单点击」触发的图（逗号分隔；默认空 = 不拦任何群）。菜单链接里的 HMAC 在
    # 客户端缓存里仍然有效，删掉群菜单**不能**阻止已缓存的按钮再次触发，故保留这份服务端黑名单兜底；
    # ``op=install|sync`` 也会拒绝往这些群装菜单。不影响卡片按钮与每日定时播报。
    # 注意：**不需要**用它来防止串群 —— 每个群的菜单链接各自带own chat_id + 独立签名，点哪个群就发哪个群。
    "CHAT_MENU_BLOCK_CHAT_IDS": "",
}
...
# time.time() of the watchdog's last successful Grafana fetch (0.0 = none yet); read by the daily health report
...
def _error_req_series_specs() -> List[Tuple[str, List[str]]]:
    ...
    if not raw:
        ...
        if MONITORING_ERROR_REQ_SERIES_A_PARTS:
            out.append((MONITORING_ERROR_REQ_SERIES_A_LABEL, MONITORING_ERROR_REQ_SERIES_A_PARTS))
        if MONITORING_ERROR_REQ_SERIES_B_PARTS:
            out.append((MONITORING_ERROR_REQ_SERIES_B_LABEL, MONITORING_ERROR_REQ_SERIES_B_PARTS))
        return out
...
GRAFANA_SCREENSHOT_ALERT_HIGHLIGHT = _lark_env_truthy("GRAFANA_SCREENSHOT_ALERT_HIGHLIGHT")
GRAFANA_SCREENSHOT_ALERT_HIGHLIGHT_COLOR = _cfg_str(
    "GRAFANA_SCREENSHOT_ALERT_HIGHLIGHT_COLOR", "#FF0000"
).strip() or "#FF0000"
...
def _lark_extract_at_entity_ids_from_im_message(
    ...
    *,
    ...
    Also walks **post** rich-text JSON ``{\"tag\": \"at\", \"user_id\": \"ou_…\"}`` (mobile), which never
    appears as HTML ``<at>`` in blobs.

    Pass the same ``mentions_list`` as IM handling (``message`` + ``event`` merged) so post ``user_id`` values
    that are ``union_id`` / internal ids map through ``mentions[]`` (HTTP often attaches rows on ``event``).
    """
...
def _monitoring_im_matches_deploy_command(clean: str) -> bool:
    if not MONITORING_DEPLOY_ENABLE:
        return False
    cn = re.sub(r"\s+", " ", (clean or "").strip().lower())
    if not cn:
        return False
    if cn == "git pull and restart service":
        return True
...
def _monitoring_deploy_repo_dir() -> str:
    raw = (MONITORING_DEPLOY_REPO_DIR or "").strip()
    if raw:
...
def _monitoring_deploy_delayed_restart(cmd: str) -> None:
    try:
        ...
        if r.returncode != 0:
            logger.error(
                "deploy restart failed exit=%s stderr=%r stdout=%r",
                r.returncode,
                (r.stderr or "")[:500],
                (r.stdout or "")[:500],
            )
    except Exception:
        logger.exception("deploy delayed restart failed")


def _monitoring_deploy_run_git_pull_and_restart() -> Tuple[bool, str]:
    repo = _monitoring_deploy_repo_dir()
    if not os.path.isdir(repo):
        return False, f"Repo directory not found: {repo}"
    ...
    if restart_cmd:
        threading.Thread(
            target=_monitoring_deploy_delayed_restart,
            args=(restart_cmd,),
            daemon=False,
            name="deploy-restart",
        ).start()
        restart_note = f"\n\nService restart scheduled ({restart_cmd})."
    else:
...
def _monitoring_deploy_worker(chat_id: str, open_id: str, debounce_key: str) -> None:
    try:
        ...
        prefix = "**Deploy OK**" if ok else "**Deploy failed**"
    ...
    try:
        ...
        h = page.evaluate(
            """() => {
              ...
              const main = document.querySelector('main');
              let h = 0;
              if (main) h = Math.max(h, main.scrollHeight || 0);
              if (rr) h = Math.max(h, rr.scrollHeight || 0);
              if (!h) h = document.body.scrollHeight || 800;
              return Math.min(Math.max(h, 800), 48000);
            }"""
        )
        h = int(h or 800)
        y = 0
...
def _grafana_playwright_scroll_to_top(page: Any) -> None:
    try:
        page.evaluate(
            """() => {
              window.scrollTo(0, 0);
...
def _grafana_playwright_compact_empty_panels(page: Any) -> int:
    """
    Shrink tall "No data" panels (e.g. Login With Password) via injected CSS only.
    Does not rewrite react-grid transforms — safe for full-page screenshots.
    """
    ...
    try:
        n = page.evaluate(
            """([capPx]) => {
              ...
              const styleId = 'grafanabot-compact-empty-style';
              ...
              const innerCap = Math.max(24, capPx - 28);
              st.textContent =
                '.grafanabot-compact-empty {' +
                'height:' + capPx + 'px !important;' +
                'max-height:' + capPx + 'px !important;' +
                'min-height:' + capPx + 'px !important;' +
                'overflow:hidden !important;}' +
                '.grafanabot-compact-empty [data-testid="panel-content"],' +
                '.grafanabot-compact-empty [data-testid="data-testid panel content"],' +
                '.grafanabot-compact-empty [data-testid="data-testid Panel content"] {' +
                'max-height:' + innerCap + 'px !important; overflow:hidden !important;}';
              let compacted = 0;
              for (const el of items) {
                ...
                if (noData && !hasChart && h > capPx + 40) {
                  el.classList.add('grafanabot-compact-empty');
                  compacted += 1;
                }
              }
              return compacted;
...
def _grafana_playwright_capture_dashboard_png(page: Any) -> bytes:
    """
    Capture full dashboard at scroll 0 after lazy panels are painted.
    Avoid clip/element capture — they truncate long dashboards (Pulsar rows) or break grid layout.

    Do **not** scroll to the bottom here unless ``GRAFANA_SCREENSHOT_PRE_CAPTURE_RESCROLL=1`` —
    a last-second scroll away from KPI / IGG panels often repaints them with incomplete
    Prometheus tails (right-edge cliff to zero) while spinners are already gone.
    """
    if GRAFANA_SCREENSHOT_PRE_CAPTURE_RESCROLL:
        try:
            page.evaluate(
                """() => {
                  const items = document.querySelectorAll('.react-grid-item');
            ...
            page.wait_for_timeout(max(300, int(GRAFANA_SCREENSHOT_SCROLL_PAUSE_MS)))
        except Exception:
            pass
...
def _grafana_resolve_alert_solo_captures(
    session: requests.Session,
    ...
    *,
    ...
    """
    If solo alert capture applies, return one ``(solo_url, panel_title, panel_id)`` per alerting panel.
    Panels without a resolvable ``panelId`` are skipped; empty list → caller falls back to full dashboard.
    """
    ...
    for title in titles:
        ...
        if pid is None:
            logger.warning(
                "Grafana alert solo: panel id not found for title=%r — skipping",
                title,
            )
            continue
        ...
        out.append((url, title, pid))
    if len(out) > 1:
        logger.info(
            "Grafana alert solo: %s alerting panels; capturing each: %s",
            len(out),
            [(t, p) for _, t, p in out],
        )
    elif len(out) == 1:
        _u, t0, p0 = out[0]
        logger.info("Grafana alert solo: panelId=%s title=%r", p0, t0)
    elif titles:
        logger.warning(
            "Grafana alert solo: no panel ids for %s title(s) — falling back to full dashboard",
            len(titles),
        )
    return out
...
def _grafana_normalize_panel_title(title: str) -> str:
    return re.sub(r"\s+", " ", (title or "").strip())


def _grafana_normalize_panel_title_for_match(title: str) -> str:
    """Casefold + unify full-width punctuation so config titles match Grafana UI."""
    s = _grafana_normalize_panel_title(title)
    s = (
        s.replace("（", "(")
        .replace("）", ")")
        .replace("【", "[")
...
def _grafana_panel_titles_equivalent(a: str, b: str) -> bool:
    na = _grafana_normalize_panel_title_for_match(a)
    nb = _grafana_normalize_panel_title_for_match(b)
    if not na or not nb:
        return False
    if na == nb:
        return True
...
def _monitoring_screenshot_highlight_titles(
    payload: Optional[Dict[str, Any]], *, alert_hit: bool
) -> List[str]:
    """
    Grafana panel title(s) for solo/highlight screenshot.

    ``/mo {panel}`` (``moPanelOnlyKind``) always targets that panel; full ``/mo`` uses solo only on alert.
    """
    ...
    if alert_hit:
        return _monitoring_alert_panel_titles(payload)
    return []
...
def _monitoring_alert_panel_titles(payload: Optional[Dict[str, Any]]) -> List[str]:
    ...
    out: List[str] = []
    for t in titles:
        n = _grafana_normalize_panel_title(t)
...
def _grafana_playwright_highlight_alert_panels(page: Any, panel_titles: Optional[List[str]]) -> int:
    """
    Draw a red border around Grafana panels whose titles match ``panel_titles``.
    Returns the number of panels highlighted (0 if disabled or none matched).
    """
    if not GRAFANA_SCREENSHOT_ALERT_HIGHLIGHT:
        return 0
    raw = [str(t).strip() for t in (panel_titles or []) if str(t).strip()]
    if not raw:
        return 0
    color = GRAFANA_SCREENSHOT_ALERT_HIGHLIGHT_COLOR
    px = int(GRAFANA_SCREENSHOT_ALERT_HIGHLIGHT_BORDER_PX)
    try:
        n = page.evaluate(
            """([titles, color, borderPx]) => {
              const norm = (s) => (s || '').replace(/\\s+/g, ' ').trim();
              const want = new Set(titles.map(norm).filter(Boolean));
              const cls = 'grafanabot-alert-highlight';
              ...
              const styleId = 'grafanabot-alert-highlight-style';
              let st = document.getElementById(styleId);
              if (!st) {
                st = document.createElement('style');
                st.id = styleId;
              ...
              st.textContent =
                '.' + cls + '{' +
                'outline:' + borderPx + 'px solid ' + color + '!important;' +
                'outline-offset:-2px!important;' +
                'box-shadow:0 0 0 ' + borderPx + 'px ' + color + '!important;' +
                'position:relative!important;z-index:5!important;}';
              let count = 0;
              const headers = document.querySelectorAll(
                'h2[title], h2, [data-testid="panel-header-title"], [class*="panel-title"]'
              );
              for (const h of headers) {
                const rawTitle = (h.getAttribute('title') || h.textContent || '').trim();
                const t = norm(rawTitle);
                let matched = want.has(t);
                if (!matched) {
                  for (const w of want) {
                    if (t.includes(w) || w.includes(t)) { matched = true; break; }
                  }
                }
                if (!matched) continue;
                const panel =
                  h.closest('.react-grid-item') ||
                  h.closest('[data-testid^="data-testid Panel"]') ||
                  h.closest('[class*="PanelChrome"]') ||
                  h.closest('[data-testid="dashboard-panel"]');
                if (panel && !panel.classList.contains(cls)) {
        ...
        if n_int:
            logger.info(
                "Grafana screenshot: highlighted %s alert panel(s): %s",
                n_int,
                raw[:6],
            )
        else:
            logger.warning(
                "Grafana screenshot: alert highlight requested but no panel DOM matched titles=%r",
                raw,
            )
        page.wait_for_timeout(120)
        return n_int
    except Exception as e:
        logger.warning("Grafana screenshot: alert panel highlight failed: %s", e)
        return 0
...
def _grafana_playwright_pre_screenshot_paint_flush(page: Any) -> None:
    ...
    Scroll-paint all panels, then return to top so element / page capture is not mid-scroll stitched.
    """
    try:
        page.evaluate(
            """async () => {
    ...
    try:
        ...
        pass
    try:
        _grafana_scroll_paint_lazy_panels(page, return_to_top=True)
        _grafana_wait_loading_like_gone(
            page,
            max(
                int(GRAFANA_SCREENSHOT_POST_REFRESH_SPINNER_MS),
                int(GRAFANA_SCREENSHOT_SPINNER_MAX_MS),
            ),
        )
        page.wait_for_timeout(220)
    except Exception:
...
def _grafana_legend_text_matches_series(want_label: str, legend_text: str) -> bool:
    ...
    if want_rpc and text_rpc and want_rpc != text_rpc:
        return False
    svc_match = want_svc == text_svc
    if not svc_match:
        longer, shorter = (
            (want_svc, text_svc) if len(want_svc) >= len(text_svc) else (text_svc, want_svc)
        )
        if len(shorter) >= 20 and longer.startswith(shorter):
            svc_match = True
    if not svc_match:
        return False
    if not want_rpc or not text_rpc:
        return True
...
def _grafana_playwright_legend_scroll_to_label(page: Any, want_label: str, *, max_rounds: int = 40) -> None:
    """Scroll the legend container until ``want_label`` appears or rounds exhausted."""
    want_cf = _grafana_legend_normalize_label(want_label).casefold()
    if not want_cf:
        return

    def _visible_has_want() -> bool:
        for row in _grafana_playwright_legend_rows_for_isolate(page):
            if _grafana_legend_labels_exact_equal(want_label, str(row.get("text") or "")):
                return True
        return False

    if _visible_has_want():
        return
    for _ in range(max(1, int(max_rounds))):
        ...
        if _visible_has_want():
            return
    # Scroll back to top then scan down once more (target may be above current view).
    try:
        page.evaluate(
            """() => {
              const leg = document.querySelector(
...
def _grafana_playwright_legend_rows_scan_all(page: Any, want_label: str) -> Dict[str, Any]:
    """Read-only full legend scan (scroll entire list). Returns target coords + visible rows."""
    want = (want_label or "").strip()
    exact_only = _grafana_want_legend_exact_only(want)
    try:
        raw = page.evaluate(
            ...
            + """
              ...
              const target = rows.find((r) => matches(r.text)) || null;
              const visible = rows.filter((r) => !r.hidden);
              return {
                target,
                visible,
                rows,
                allTexts: rows.map((r) => r.text),
              };
            }""",
            {"want": want, "exactOnly": exact_only},
        )
        ...
        logger.warning("Grafana screenshot: legend scan failed want=%r: %s", want[:80], e)
        return {}


def _grafana_playwright_legend_row_click_coords(
    page: Any, row_text: str, *, scroll_from_top: bool = True
) -> Optional[Dict[str, Any]]:
    """
    Scroll the legend until ``row_text`` is visible and return **fresh** click coordinates.

    Coordinates from :func:`_grafana_playwright_legend_rows_scan_all` go stale once the scroller
    moves — always use this immediately before ``page.mouse.click`` / ``dblclick``.
    """
    want = (row_text or "").strip()
    if not want:
        return None
    exact_only = _grafana_want_legend_exact_only(want)
    try:
        raw = page.evaluate(
            ...
            + """
              ...
              const collectRows = () => {
                ...
                for (const sel of sels) {
                  document.querySelectorAll(sel).forEach((el) => {
                    ...
                    const r = tgt.getBoundingClientRect();
                    if (r.width < 2 || r.height < 2) return;
                    out.push({ text, x: r.left + r.width / 2, y: r.top + r.height / 2, el: tgt });
                  });
                }
                return out;
              };
              ...
              const sc = scroller();
              if (sc && scrollFromTop) sc.scrollTop = 0;
              const maxSteps = sc ? 150 : 1;
            ...
            {"want": want, "exactOnly": exact_only, "scrollFromTop": scroll_from_top},
        )
        if isinstance(raw, dict) and raw.get("x") is not None:
            return raw
    except Exception as e:
        logger.warning(
            "Grafana screenshot: legend row coords failed want=%r: %s", want[:80], e
        )
    return None
...
def _grafana_playwright_legend_hide_all_except(
    page: Any, want_label: str, all_texts: List[str]
) -> bool:
    """Single-click every other **visible** legend series (full list) to hide them."""
    want = (want_label or "").strip()
    if not want:
        return False
    exact_only = _grafana_want_legend_exact_only(want)
    ...
    for txt in all_texts:
        label = str(txt or "").strip()
        if not label or _legend_matches(label):
            continue
        key = label.casefold()
...
def _grafana_playwright_try_isolate_legend_series(
    ...
    for label in ordered:
        if _grafana_playwright_isolate_legend_via_mouse(
            ...
            return True
        logger.info(
            "Grafana screenshot: legend isolate miss candidate=%r", label[:100]
        )
    return False
...
def _grafana_playwright_isolate_legend_via_mouse_OLD(
    ...
    def _verified() -> bool:
        if _grafana_playwright_legend_isolate_verified(page, want):
            return True
        vis = _grafana_playwright_legend_visible_labels(page)
        matched = [v for v in vis if _legend_matches(v)]
        return len(vis) == 1 and len(matched) == 1
    ...
    try:
        ...
        locator_candidates = [
            ...
            legend_root.locator("table.viz-legend tbody tr").filter(
                has_text=re.compile(rf"^{esc}$", re.I)
            ),
            legend_root.get_by_text(want, exact=True),
        ]
        ...
        # 3) Retry hide passes on whatever is still visible (virtualized legend safety net).
        for hide_pass in range(4):
            vis_labels = _grafana_playwright_legend_visible_labels(page)
            if len(vis_labels) == 1 and any(_legend_matches(v) for v in vis_labels):
                logger.info(
                    "Grafana screenshot: hide-others isolate ok want=%r pass=%s",
                    want[:100],
                    hide_pass,
                )
                return True
            scan2 = _grafana_playwright_legend_rows_scan_all(page, want)
            texts2 = scan2.get("allTexts") if isinstance(scan2.get("allTexts"), list) else vis_labels
        ...
        logger.warning(
            "Grafana screenshot: mouse isolate failed want=%r target=%r visible=%s legend_count=%s",
            want[:100],
            target_txt[:120],
            _grafana_playwright_legend_visible_labels(page)[:12],
            len(all_texts),
        )
    except Exception as e:
        logger.warning("Grafana screenshot: mouse isolate error want=%r: %s", want[:80], e)
    return False
...
def _grafana_playwright_collect_legend_click_targets(page: Any) -> List[Dict[str, Any]]:
    """Return ``[{text, x, y, from_testid?}, …]`` for Grafana viz legend rows (testid-first)."""
    raw = page.evaluate(
        """() => {
          const out = [];
          const seen = new Set();
          ...
          const push = (el, fromTestid) => {
            ...
            out.push({
              ...
              x: r.left + r.width / 2,
              y: r.top + r.height / 2,
              from_testid: !!fromTestid,
            });
          };
          document.querySelectorAll('[data-testid*="VizLegend series"]').forEach((el) => {
            push(el, true);
          });
          ...
          for (const sel of sels) {
            document.querySelectorAll(sel).forEach((el) => push(el, false));
          }
          return out;
        }"""
    )
...
def _grafana_playwright_legend_rows_for_isolate(page: Any) -> List[Dict[str, Any]]:
    ...
    raw = page.evaluate(
        ...
        + """
          ...
          if (leg) leg.scrollIntoView({ block: 'end', behavior: 'instant' });
...
def _grafana_playwright_chart_ymax_approx(page: Any) -> float:
    """Best-effort top Y-axis tick from the solo panel (0 if unreadable)."""
    try:
        v = page.evaluate(
            """() => {
              let max = 0;
              ...
              document.querySelectorAll('svg text, [class*="axis"] text, .uplot text').forEach((el) => {
...
def _grafana_alert_isolate_series_labels(
    ...
    for kind, g_title, keyword, analyzer, http_only in _grafana_alert_panel_specs():
        if not _grafana_panel_titles_equivalent(title, g_title):
            continue
        if _monitoring_alert_channel_muted(kind):
            return []
...
def _grafana_playwright_render_solo_panel_and_png(
    ...
    *,
    ...
    When isolate labels are set, double-click that legend row first (one series only).
    """
    page.goto(url, wait_until="domcontentloaded", timeout=timeout_ms)
    try:
    ...
    for raw in list(isolate_series_candidates or []) + (
        [isolate_series_label] if isolate_series_label else []
    ):
        label = (raw or "").strip()
        if not label:
            continue
    ...
    if iso_candidates:
        ...
        if not isolated:
            logger.warning(
                "Grafana screenshot: legend isolate failed — PNG may show wrong series candidates=%s",
                [c[:80] for c in iso_candidates[:6]],
            )
        elif GRAFANA_SCREENSHOT_SETTLE_MS > 0:
            page.wait_for_timeout(min(400, int(GRAFANA_SCREENSHOT_SETTLE_MS)))
...
class GrafanaPlaywrightKeeper:
    ...
    def __init__(self) -> None:
        ...
        # Job outcomes for the daily health card: a Chromium that dies after warm-up only shows here.
        self._last_job_at = 0.0
        self._job_fail_streak = 0
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    self._job_fail_streak = self._job_fail_streak + 1 if "err" in box else 0
                    ev.set()
        except Exception as ex:
            self._fatal = ex
...
def _grafana_headless_screenshot_png_at_url(
    session: requests.Session,
    url: str,
    *,
    highlight_panel_titles: Optional[List[str]] = None,
...
def _grafana_headless_screenshot_png_list(
    session: requests.Session,
    start_unix: int,
    end_unix: int,
    *,
    ...
    ``GRAFANA_SCREENSHOT_FULL_PAGE=1`` (default): full dashboard for ``/mo`` / non-alert paths (one PNG).
...
def _grafana_headless_screenshot_png(
    ...
    *,
    ...
    """First PNG from :func:`_grafana_headless_screenshot_png_list` (backward-compatible single capture)."""
    ...
    if not pngs:
        raise RuntimeError("Grafana screenshot produced no PNG bytes")
    return pngs[0]
...
def _grafana_watchdog_alert_screenshot_png(
    ...
    pngs = _grafana_watchdog_alert_screenshot_png_list(session, payload)
    if not pngs:
        raise RuntimeError("watchdog alert screenshot produced no PNG bytes")
    return pngs[0]


def _lark_send_monitoring_alert_pngs(
    receive_id_type: str,
    receive_id: str,
    pngs: List[bytes],
    *,
    embedded_in_card: bool = False,
...
def _monitoring_send_screenshot_on_card_click(chat_id: str, open_id: str) -> None:
    try:
        ...
        if (chat_id or "").strip():
            recv_type, recv_id = "chat_id", chat_id.strip()
        elif (open_id or "").strip():
            recv_type, recv_id = "open_id", open_id.strip()
        else:
            raise RuntimeError("missing chat_id/open_id")
        n_sent = _lark_send_monitoring_alert_pngs(recv_type, recv_id, pngs)
        logger.info(
            "monitoring card-action screenshot sent chat=%r open=%r images=%s bytes=%s alert_hit=%s highlight_panels=%s",
            bool(chat_id),
            bool(open_id),
            n_sent,
            sum(len(p) for p in pngs),
            alert_hit,
            len(highlight),
        )
    except Exception as e:
...
def _monitoring_ai_abnormal_verdict(
    ...
    # Hard cap on generated tokens: an uncapped qwen3 can keep emitting (thinking not always
    # suppressed) and burn the whole timeout on one verdict. The answer we need is ~2 short lines.
    n_pred = _cfg_int("MONITORING_AI_NUM_PREDICT", 256)
    if n_pred > 0:
        opts["num_predict"] = int(n_pred)
    body: Dict[str, Any] = {
        "model": model,
        "stream": False,
...
def _monitoring_watchdog_deliver_alert(
    ...
    if not ai_ok:
        if health_report is not None:
            health_report.bump("Watchdog alerts suppressed by AI gate")
        logger.info(
    ...
    if _lark_env_truthy_or_default("MONITORING_ALERT_ONE_CARD_PER_PANEL", default=True):
        ...
        if n_cards > 0:
            ...
            if health_report is not None:
                health_report.bump("Watchdog alerts sent")
            return True
        logger.warning(
    ...
    if health_report is not None:
        health_report.bump("Watchdog alerts sent")  # card/text is out; the extra PNGs below are best-effort
    if _lark_env_truthy("GRAFANA_SCREENSHOT_ENABLE"):
        try:
...
# ---------------------------------------------------------------------------
# Daily health report (shared health_report.py): one status card a day to the ops group.
# Its HEALTH_REPORT_* knobs are read from os.environ (systemd Environment=), not _CFG; module
# defaults are 09:00 UTC+8 to oc_ad9b… (Laboratory). The checks below are read-only: they
# report in-memory state or make one short GET / token call — never send, restart or log in.
# ---------------------------------------------------------------------------
_health_boot_at = time.time()


def _health_host(url: str) -> str:
    """Hostname only — a full URL could carry credentials."""
...
def _health_check_lark_api() -> Tuple[Any, str]:
    """Fresh tenant-token fetch; does not touch the send path's token cache."""
    if not APP_ID or not APP_SECRET:
        return "fail", "APP_ID / APP_SECRET not configured"
    base = _lark_api_domain()
    host = _health_host(base)
    t0 = time.monotonic()
    try:
        r = requests.post(
        ...
        return "fail", f"{host}: {e.__class__.__name__}"
    ...
    if not isinstance(j, dict):
        return "fail", f"{host}: HTTP {r.status_code}, non-JSON reply"
    if r.status_code != 200 or j.get("code") != 0 or not j.get("tenant_access_token"):
        return "fail", f"{host}: HTTP {r.status_code}, code {j.get('code')} {str(j.get('msg') or '')[:80]}".rstrip()
    return "ok", f"{host}: tenant token OK in {ms:.0f} ms"


def _health_check_webhook_server() -> Tuple[Any, str]:
    """Local GET /health on the listener that takes Lark events (http mode) and card callbacks."""
    import sys

    mode = _cfg_str("LARK_EVENT_MODE", "http").strip().lower() or "http"
    if mode == "ws" and _cfg_str("ENABLE_HTTP", "1").strip().lower() not in ("1", "true", "yes", "on"):
        return None, "disabled by ENABLE_HTTP"
    port = _cfg_listen_port()
    t0 = time.monotonic()
    try:
        r = requests.get(f"http://127.0.0.1:{port}/health", timeout=5)
    except Exception as e:
        return "fail", f"port {port}: {e.__class__.__name__}"
    ms = (time.monotonic() - t0) * 1000.0
    if r.status_code != 200:
        return "fail", f"port {port}: HTTP {r.status_code}"
    stack = _cfg_str("HTTP_SERVER", "flask").strip().lower()
    # run_http() imports waitress lazily and serves Flask when that import fails (one boot warning only).
    if stack in ("waitress", "wsgi") and "waitress" not in sys.modules:
        return "warn", f"port {port}: HTTP 200 in {ms:.0f} ms — waitress not installed, Flask fallback serving ({mode} mode)"
    # Feishu gives callbacks ~1-3s end to end; a slow local /health means the thread pool is saturated.
    return ("ok" if ms < 1000 else "warn"), f"port {port}: HTTP 200 in {ms:.0f} ms ({stack}, {mode} mode)"


def _health_check_grafana() -> Tuple[Any, str]:
    """Grafana's unauthenticated /api/health — no login, no dashboard query."""
    ...
    ver = str(j.get("version") or "")[:20]
    return ("ok" if db == "ok" else "warn"), f"{host}: HTTP 200 in {ms:.0f} ms, database {db}" + (
        f", v{ver}" if ver else ""
    )


def _health_check_watchdog() -> Tuple[Any, str]:
    """Age of the watchdog's last successful Grafana fetch."""
    if not _monitoring_watch_started:
        return None, "disabled by MONITORING_WATCH_ENABLE"
    sec = max(15.0, _cfg_float("MONITORING_WATCH_INTERVAL_SECONDS", 20.0))
    # A cycle that raises an alert blocks for minutes (confirm wait, screenshots, AI review) — hence the floors.
    warn_after = max(3 * sec, 600.0)
    fail_after = max(15 * sec, 1800.0)
    now = time.time()
    last = _monitoring_watch_last_ok_at
    if _monitoring_watch_in_daily_quiet_local(now):
        return "ok", "daily quiet window, not polling" + (f"; last poll {_fmt_duration(now - last)} ago" if last else "")
    if not last:
        up = now - _health_boot_at
        if up < warn_after:
            return "ok", f"no poll yet (started {_fmt_duration(up)} ago)"
        return "fail", f"no successful Grafana poll since boot {_fmt_duration(up)} ago"
    age = now - last
    detail = f"last successful poll {_fmt_duration(age)} ago (every {sec:.0f}s)"
    if age > fail_after:
        return "fail", detail
    if age > warn_after:
        return "warn", detail
...
def _health_check_ollama() -> Tuple[Any, str]:
    """AI-gate model server: the ai-health watchdog's latest verdict (<= 1 interval old), else a /api/tags ping."""
    if not _lark_env_truthy_or_default("MONITORING_AI_GATE_ENABLE", default=True):
        return None, "disabled by MONITORING_AI_GATE_ENABLE"
    model = _cfg_str("MONITORING_AI_MODEL", "qwen3.6:35b-a3b").strip()
    url = _cfg_str("MONITORING_AI_OLLAMA_URL", "http://localhost:11434").strip().rstrip("/")
    host = _health_host(url)
    with _ai_health_lock:
        st = dict(_ai_health_state)
    healthy = st.get("healthy")
    detail = str(st.get("last_detail") or "")
    detail = (detail.replace(url, host) if url else detail)[:140]
    now = time.time()
    if healthy is True:
        fails = int(st.get("fails") or 0)
        if fails:
            return "warn", f"{model}: last probe failed ({fails}x) — {detail}"
        return "ok", f"{model}: {detail or 'healthy'}"
    if healthy is False:
        down = _fmt_duration(now - float(st.get("down_since") or now))
        restart = float(st.get("last_restart") or 0.0)
        tail = f"; last restart {_fmt_duration(now - restart)} ago" if restart else ""
        return "fail", f"{model}: down for {down} — {detail}{tail}"
    # Not probed yet, or MONITORING_AI_HEALTH_ENABLE=0: just check the daemon answers.
    t0 = time.monotonic()
    try:
        ...
        return "fail", f"{host}: {e.__class__.__name__}"
    if r.status_code != 200:
        return "fail", f"{host}: HTTP {r.status_code}"
    return "ok", f"{host}: daemon up in {(time.monotonic() - t0) * 1000.0:.0f} ms (no model probe yet)"


def _health_check_chromium() -> Tuple[Any, str]:
    """Persistent Playwright Chromium that renders alert / menu screenshots."""
    if not _grafana_persistent_browser_enabled():
        return None, "disabled by GRAFANA_SCREENSHOT_ENABLE / GRAFANA_PERSISTENT_BROWSER"
    k = _grafana_pw_keeper
    if k is None:
        return "fail", "keeper not started; screenshots launch a new browser each time"
    fatal = k._fatal
    if isinstance(fatal, ImportError):
        return "fail", "Playwright not installed"
    if fatal is not None:
        return "fail", f"keeper crashed ({fatal.__class__.__name__}); screenshots fall back to a one-off browser"
    if k._thread is None or not k._thread.is_alive():
        return "fail", "keeper thread not running"
    if not k._ready.is_set():
        up = time.time() - _health_boot_at
        # request_png itself waits this long for the warm-up before giving up.
        warm = max(120.0, float(GRAFANA_SCREENSHOT_TIMEOUT_MS) / 1000.0 + 45.0)
        if up < 3 * warm:
            return "ok", f"warming up (started {_fmt_duration(up)} ago)"
        return "warn", f"warm-up not finished after {_fmt_duration(up)}"
    q = k._q.qsize()
    streak = k._job_fail_streak
    if streak:
        ago = _fmt_duration(time.time() - k._last_job_at)
        return "warn", f"last {streak} screenshot job(s) failed, latest {ago} ago; screenshots fall back to a one-off browser"
    return ("ok" if q < 5 else "warn"), f"Chromium ready, {q} screenshot job(s) queued"


def _health_check_alert_mutes() -> Tuple[Any, str]:
    """Active /m mutes silence watchdog alerts; flag one that still has more than 12 h to run (longest choice is 1 day)."""
    now = time.time()
    graphs = {k: float(v or 0.0) for k, v in dict(_MONITORING_MUTE_UNTIL).items() if float(v or 0.0) > now}
    series = [float(v or 0.0) for v in dict(_MONITORING_SERIES_MUTE_UNTIL).values() if float(v or 0.0) > now]
    if not graphs and not series:
        return "ok", "none active"
    parts: List[str] = []
    if graphs:
        names = ", ".join(_mute_channel_display_label(k) for k in sorted(graphs))
        parts.append(f"{len(graphs)} graph(s) ({names[:90]})")
    if series:
        parts.append(f"{len(series)} series")
    left = max(list(graphs.values()) + series) - now
    detail = f"{' + '.join(parts)} muted; longest has {_fmt_duration(left)} left"
    return ("warn" if left > 12 * 3600 else "ok"), detail


def _health_report_send_card(chat_id: str, card: Dict[str, Any]) -> Any:
    """Post the report as a fresh message via the module's Lark sender with this bot's app.

    Unlike ``_lark_send_interactive_card`` (``raise_for_status`` drops Lark's code on HTTP 400) it
    raises LarkError(code), so 230002 / 99991672 stop the retries with a hint, and it carries the
    report's Lark uuid, so a retry after a timed-out send that did arrive posts no second card.
    Built per call so it follows ``_lark_api_domain()`` (the WS region override); never threaded."""
    return health_report.make_lark_sender(
        str(APP_ID or "").strip(), str(APP_SECRET or "").strip(), _lark_api_domain()
    )(chat_id, card)


def _health_report_expected_threads() -> List[str]:
    """Long-lived threads the boot starters launched in this configuration (one-shot warmers excluded)."""
    gate = _lark_env_truthy_or_default("MONITORING_AI_GATE_ENABLE", default=True)
    names: List[str] = []
    if _grafana_pw_keeper is not None:
        names.append("grafana-playwright-keeper")
    if (
        gate
        and _lark_env_truthy_or_default("MONITORING_AI_WARM_ENABLE", default=True)
        and _cfg_float("MONITORING_AI_WARM_INTERVAL_SECONDS", 1200.0) > 0
    ):
        names.append("ai-warm")
    if gate and _lark_env_truthy_or_default("MONITORING_AI_HEALTH_ENABLE", default=True):
        names.append("ai-health")
    if _monitoring_watch_started:
        names.append("monitoring-watchdog")
    if _freespin_daily_started:
        names.append("freespin-daily")
    mode = _cfg_str("LARK_EVENT_MODE", "http").strip().lower() or "http"
    if mode == "ws" and _cfg_str("ENABLE_HTTP", "1").strip().lower() in ("1", "true", "yes", "on"):
        names.append("http-sidecar")
    return names


def _start_daily_health_report_if_enabled() -> None:
    """Start the daily health card thread (HEALTH_REPORT_ENABLE=0 turns it off); never breaks boot."""
    if health_report is None:
        logger.warning("health report: health_report.py not importable (%s) — daily report off", _health_report_import_error)
        return
    try:
        health_report.start(
            "monitoring",
            ...
            checks=[
                ...
                ("Webhook server", _health_check_webhook_server),
                ("Grafana", _health_check_grafana),
                ("Grafana watchdog", _health_check_watchdog),
                ("Ollama (AI gate)", _health_check_ollama),
                ("Chromium (screenshots)", _health_check_chromium),
                ("Alert mutes", _health_check_alert_mutes),
            ],
            expect_threads=_health_report_expected_threads(),
        )
    except Exception:
        logger.exception("health report: start failed — bot keeps running without it")
                ...
                if not _lark_env_truthy("GRAFANA_SCREENSHOT_ENABLE"):
                    ...
                    if su <= 0 or eu <= 0:
                        ...
                        try:
                            ...
                            highlight = _monitoring_screenshot_highlight_titles(
                                payload, alert_hit=alert_hit
                            )
                            pngs = _grafana_headless_screenshot_png_list(
                                grafana_session,
                                su,
                            ...
                            n_img = _lark_send_monitoring_alert_pngs(
                                ...
                                recv_id,
                                pngs,
                                embedded_in_card=embedded_png_in_card,
                                pre_key=pre_key,
                            )
                            if embedded_png_in_card and len(pngs) <= 1:
                                logger.info(
                                    "monitoring: Grafana PNG embedded in interactive card — extra images=%s",
                                    max(0, len(pngs) - 1),
                                )
                            logger.info(
                                "monitoring Grafana screenshot sent (background) images=%s bytes=%s",
                                n_img,
                                sum(len(p) for p in pngs),
                            )
                        except Exception:
    ...
    if _monitoring_im_is_deploy_command(raw_text or "", clean or ""):
        ...
        with _monitoring_reply_dispatch_lock:
            if im_event_id and im_event_id in _processed_lark_im_event_ids:
                ...
                return
            if processed_d and processed_d in _processed_lark_message_ids:
                logger.info("duplicate deploy stick=%r — skip", processed_d[:96])
                return
            ...
            if processed_d:
                _processed_lark_message_ids.add(processed_d)
            if im_event_id:
                _processed_lark_im_event_ids.add(im_event_id)
                if len(_processed_lark_im_event_ids) > _PROCESSED_IM_EVENT_IDS_CAP:
                    _processed_lark_im_event_ids.clear()
